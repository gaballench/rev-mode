...
(defun process-rb ()
  "returns a list with existing rb process."
  (interactive)
  ...
  (while (< i lengthpr)
    ...
    (if pos (add-to-list 'listrbp (process-name (get-process (elt listpr i)))))
    (setq i (+ 1 i)))
  listrbp)
...
(setq rblist (process-rb))
(setq rbn (length rblist))
(if (eq rbn 0)
    (progn (rb)
	   ...
	   (sleep-for 0.5)))
(if (eq rbn 1)
    (setq outpr (get-process (elt rblist 0))))
(if (> rbn 1)
(progn
(setq proc (completing-read "Send code to:" rblist nil t (elt rblist 0)))
(setq outpr (get-process proc))))
outpr)


(defun rb-eval-line (sprocess command)
  "Evaluates a single command into the rb process."
  ...
  (with-current-buffer sbuffer 
    ...
    (insert command)			;pastes the command to rb
    (set-marker (process-mark sprocess) (point-max))
    (process-send-string sprocess command)
    ;; (accept-process-output sprocess 0 10)
    ))

; maybe unnecessary? uncomment if needed 
(defun rb-cd-current-directory ()
  "Changes the rb working directory to the current buffer's one."
  (interactive)
...
(defun pipe-line-to-rb (&optional step)
  "Evaluates the current line to the rb."
  (interactive ())
...
(defun pipe-line-to-rb-and-step ()
  "Evaluates the current line to the rb and goes to next line."
  (interactive)
  (pipe-line-to-rb t))

(defun pipe-region-to-rb (start end)
  "Sends a region to the rb."
  (interactive "r")
...
(defun pipe-buffer-to-rb ()
  "Evaluate whole buffer to the rb."
  (interactive)
  (pipe-region-to-rb (point-min) (point-max)))

(defun pipe-function-to-rb ()
"Evaluate function to the rb."
(interactive)
(setq beg-end (rb-beg-end-of-function))
(if beg-end
    (save-excursion
